...
function PickLocation({ onPickLocation }: { onPickLocation?: (coords: { lat: number; lng: number }) => void }) {
  useMapEvents({
    click: (event) => onPickLocation?.({ lat: event.latlng.lat, lng: event.latlng.lng }),
  });
    ...
    <div className="map-shell" style={{ borderColor: accent }}>
      ...
      >
        ...
        {markers.map((marker, idx) => (
          <Marker
            key={`${marker.subjectId ?? marker.title}-${idx}`}
            position={[marker.lat, marker.lng]}
            icon={markerIcon(marker.color ?? accent)}
          >
            <Popup>
              <div className="map-infowindow">
                <p className="map-pin-label">Sujet {marker.label ?? '—'}</p>
                <h4>{marker.title}</h4>
                {(marker.committee || marker.year) && (
                  <p className="map-hint">{[marker.committee, marker.year].filter(Boolean).join(' · ')}</p>
                )}
                {marker.subjectId && (
                  <button
                    className="map-infowindow-btn"
                    onClick={() => onSelectSujet?.(marker.subjectId as string)}
                  >
                    Voir la demande
                  </button>
                )}
